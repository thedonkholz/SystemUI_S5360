...
public class StatusBarPolicy {
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (action.equals("com.sec.android.service.statusbar.COLLAPSE")) {
                mService.collapse();
            } else if (action.equals("com.android.systemui.statusbar.SHOW_NOT_DIS_CHARGED_POPUP")) {
                showDisChargePopup(mIsDisCharged);
            }
        }
    };
    ...
    public StatusBarPolicy(Context context) {
        ...
        filter.addAction("com.sec.android.service.statusbar.COLLAPSE");
        filter.addAction("com.android.systemui.statusbar.SHOW_NOT_DIS_CHARGED_POPUP");
    ...
    private final void updateBattery(Intent intent) {
        ...
        Slog.i(TAG, "BAT. S:" + batteryStatus + " H:" + batteryHealth);
        if (batteryStatus == 3) {
            if (mIsDisCharged) {
                mIsDisCharged = false;
                if (mDisChargeDialog != null) {
                    mDisChargeDialog.dismiss();
                }
                if (mBattIntent != null) {
                    ((AlarmManager) mContext.getSystemService("alarm")).cancel(mBattIntent);
                }
            }
            if (mIsFullCharged) {
                mIsFullCharged = false;
                removeFullChargeNotification();
            }
        } else if (batteryStatus == 4) {
            if (batteryHealth == 3 || batteryHealth == 7) {
                if (!mIsDisCharged && mDisChargeDialog == null) {
                    showDisChargePopup(mIsDisCharged);
                }
                mIsDisCharged = true;
            }
        ...
        } else if (mIsDisCharged && batteryHealth == 2) {
            mIsDisCharged = false;
            if (mDisChargeDialog != null) {
                mDisChargeDialog.dismiss();
            }
    ...
    private void showLowBatteryWarning() {
        if (Settings.System.getInt(mContext.getContentResolver(), "SHOULD_SHUT_DOWN", 0) != 1) {
            CharSequence levelText;
            closeLastBatteryView();
            if (mBatteryLevel > 5) {
                levelText = mContext.getString(R.string.lowbattery);
            } else {
                levelText = this.mContext.getString(R.string.critical_lowbattery);
            }
            if (mBatteryLevelTextView != null) {
                ...
                d.setOnDismissListener(this.mLowBatteryListener);
    ...
    private void showDisChargePopup(boolean isMute) {
        Slog.d(TAG, "showDisChargePopup() " + isMute);
        mWL = ((PowerManager) mContext.getSystemService("power")).newWakeLock(6, TAG);
        mWL.acquire();
        CharSequence disChargeText = mContext.getString(R.string.dischargebattery);
        if (mDisChargeTextView != null) {
            mDisChargeTextView.setText(disChargeText);
        } else {
            View v = View.inflate(mContext, R.layout.battery_discharge, null);
            mDisChargeTextView = (TextView) v.findViewById(R.id.dischargetext);
            mDisChargeTextView.setText(disChargeText);
            AlertDialog.Builder b = new AlertDialog.Builder(mContext);
            b.setCancelable(false);
            b.setTitle(R.string.dischargetitle);
            b.setView(v);
            b.setIcon(android.R.drawable.ic_dialog_alert);
            b.setPositiveButton(android.R.string.ok, null);
            AlertDialog d = b.create();
            d.setOnDismissListener(mDisChargeListener);
            d.getWindow().setType(WindowManager.LayoutParams.TYPE_SYSTEM_ALERT);
            d.show();
            mDisChargeDialog = d;
        }
    ...
    private void playTone() {
        ...
        NotificationManager tNM = (NotificationManager) mContext.getSystemService("notification");
        Notification tN = new Notification();
        if (ringerMode == 2) {
            tN.defaults |= 1;
        } else if (ringerMode == 1) {
            tN.defaults |= 2;
        }
        tNM.notify(117506049, tN);
    }
    ...
    private DialogInterface.OnDismissListener mDisChargeListener
            = new DialogInterface.OnDismissListener() {
        public void onDismiss(DialogInterface dialog) {
            Slog.d(TAG, "mDisChargeListener");
            mDisChargeDialog = null;
            mDisChargeTextView = null;
            mWL.release();
            mWL = null;
            if (mIsDisCharged) {
                long when = SystemClock.elapsedRealtime() + 60000;
                mBattIntent = PendingIntent.getBroadcast(mContext, 0, new Intent("com.android.systemui.statusbar.SHOW_NOT_DIS_CHARGED_POPUP"), 268435456);
                ((AlarmManager) mContext.getSystemService("alarm")).set(2, when, mBattIntent);
			}
        }
    };
    ...
    private void addFullChargeNotification() {
        CharSequence string = mContext.getString(R.string.fullchargetitle);
        CharSequence string2 = mContext.getString(R.string.fullchargebattery);
        Notification notification = new Notification(R.drawable.stat_sys_battery_full, string2, 0);
        notification.flags |= 2;
        notification.setLatestEventInfo(mContext, string, string2, PendingIntent.getActivity(mContext, 0, new Intent(), 0));
        ((NotificationManager) mContext.getSystemService("notification")).notify(117506048, notification);
        playTone();
        turnOnScreenWithForce();
    }
